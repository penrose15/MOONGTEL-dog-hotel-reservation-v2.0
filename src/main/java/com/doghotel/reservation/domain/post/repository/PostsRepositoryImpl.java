...
public class PostsRepositoryImpl implements PostsRepositoryCustom{
    ...
    public Page<PostsResponsesDto> getMainPages(Pageable pageable) {
        List<PostsResponsesDto> mainPages = queryFactory
                .select(Projections.constructor(PostsResponsesDto.class,postsImg.postsImgId.min(), postsImg.name, postsImg.url, posts.title, posts.score, room.price.min()))
                .from(posts)
                .join(postsImg)
                .on(posts.id.eq(postsImg.posts.id))
                .join(room)
                .on(posts.company.companyId.eq(room.company.companyId))
                .where(postsImg.postsImgId.eq(select(postsImg.postsImgId.min())
                        .from(posts)
                        .join(postsImg)
        ...
        JPQLQuery<PostsResponsesDto> count = queryFactory
                .select(Projections.constructor(PostsResponsesDto.class,postsImg.postsImgId, postsImg.name, postsImg.url, posts.title, posts.score, room.price.min()))
                .from(posts)
                .join(postsImg)
                .on(posts.id.eq(postsImg.posts.id))
                .where(postsImg.postsImgId.eq(select(postsImg.postsImgId.min())
    ...
    public Page<PostsResponsesDto> searchPagesByTitleOrContentOrAddress(String keyword, Pageable pageable) {
        List<PostsResponsesDto> mainPages = queryFactory
                .select(Projections.constructor(PostsResponsesDto.class,postsImg.postsImgId, postsImg.name, postsImg.url, posts.title, posts.score, room.price.min())).distinct()
                .from(posts)
                .join(postsImg)
                .on(posts.id.eq(postsImg.posts.id))
                .join(room)
